...
def xy2ij(pyptlist, xdim, ydim, xmin, ymin):
    ...
    return pdict
#================================================================================
#INSTRUCTION: SPECIFY THE PTS & INPUTS
#================================================================================
xdim = 1.0
ydim = 1.0
zdim = 1.0
...
#INSTRUCTION: SPECIFY THE PTS & INPUTS
#================================================================================
csv_titles = ["eLAI@", "eleaf_area(m2)@", "grd_area(m2)@", "gap_frac@", "ext_coeff@"]
...
for j in range(1):
    ...
    tree_number = j+66
    tree_name = "tree" + str(tree_number)
    pts_file = "F:\\kianwee_work\\smart\\may2017-oct2017\\tree_modelling\\pts\\" + tree_name + "\\" + tree_name + ".pts"
    result_dir = "F:\\kianwee_work\\smart\\may2017-oct2017\\tree_modelling\\pts\\" + tree_name + "\\voxel\\"
    ...
    dae_filepath = os.path.join(result_dir, "voxels.dae")
    voxel_area = xdim*ydim
    
    lai_csv_filepath = os.path.join(result_dir, "lai.csv")
    lai_str = "tree_name\neLAI\neleaf_area(m2)\n"
    lai_file = open(lai_csv_filepath, "w")
    lai_file.write(lai_str)
    lai_file.close()

    print "READING POINT CLOUDS OF ...", tree_name
    pf = open(pts_file, "r")
    lines = pf.readlines()
    ...
    print tree_name, "XSIZE:", xmax0-xmin0
    print tree_name, "YSIZE:", ymax0-ymin0
    print tree_name, "HEIGHT:", zmax0-zmin0
    ...
    for i in range(18):
        print "#==============================="
        print "CALCULATING ...", tree_name, "...",i+1, "OUT OF 18 DIRECTIONS"
        print "#==============================="
        rot = (i+1)*10
...
#        print "N KVOXELS:", intervalk
        ...
        for cnt in range(intervalk):
                    ...
                    try:
                        ...
                        if is_collinear:
                            ...
                            for pt in ptlist:
                                ...
                                proj_ptlist.append(ppt)
                            pptlist.extend(proj_ptlist)
                            ptri = py3dmodel.construct.delaunay3d(proj_ptlist, tolerance=1e-12)
            ...
            base_2dlist[cnt].append(nvoxel_occ*voxel_area)
            
        print "NUMBER OF VOXELS:", len(voxel_list), ",GRID SIZE[X,Y,Z]:[", intervali, ",", intervali, ",", intervalk, "]"
    ...
    for kcnt in range(intervalk):
        ...
        print "EXTINCTION COEFFICIENT@", k_height, extinction_coef
        ...
        print "GAP FRACTION@", k_height, gap_fraction
        ...
        tree_info.append(lai_k)
        tree_info.append(leaf_areak)
        tree_info.append(avg_barea)
        tree_info.append(extinction_coef)
        tree_info.append(gap_fraction)
        
        
        print "EFFECTIVE LAI@",k_height, lai_k, "EFFECTIVE LEAF AREA@",k_height, leaf_areak
        
    print "EFFECTIVE LEAF AREA", leaf_area, "EFFECTIVE LEAF AREA INDEX", lai_e
    
    tree_info.insert(1, lai_e)
    tree_info.insert(2, leaf_area)
    
    lai_file = open(lai_csv_filepath, "r")
    reader = csv.reader(lai_file)
    rows_2dlist = []
    for row in reader:
        rows_2dlist.append(row)
    lai_file.close()
    
    n_rows = len(rows_2dlist)
    curr_lvl = (n_rows-3)/5
    n_info = len(tree_info)
    
    if n_info > n_rows:
        n_diff = n_info-n_rows
        for icnt in range(n_diff):
            lai_h = ((icnt/5)+curr_lvl)*zdim
            title = csv_titles[icnt%5]
            new_row = [title+str(lai_h)+"(m)"]
            #for _  in range(j):
            #    new_row.append("")
            rows_2dlist.append(new_row)
            
    for a in range(n_info):
        rows_2dlist[a].append(tree_info[a])
    
    
    lai_file = open(lai_csv_filepath, "w")
    writer = csv.writer(lai_file, lineterminator='\n')
    writer.writerows(rows_2dlist)
    lai_file.close()
    ...
    py3dmodel.export_collada.write_2_collada(dae_filepath, occface_list = voxel_list)
