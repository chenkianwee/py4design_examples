...
#imp_shp_file = "F:\\kianwee_work\\princeton\\2019_06_to_2019_12\\campus_as_a_lab\\model3d\\shp\\main_campus_imps\\main_campus_imps_test.shp"
#===========================================================================================
#FUNCTION
#===========================================================================================
...
def common_road_frm_grid(grid, road_srf_list):
    ...
    for r in road_srf_list:
        ...
        for f in common_faces:
            if common_holes:
                faces = diff_srfs_frm_grid(f, common_holes, mv_down = -10, extrude = 20)
...
def write_poly_shpfile(occface_list, shp_filepath):
    ...
    for occface in occface_list:
        ...
        if nwires > 1:
            ...
            verts = py3dmodel.fetch.topo_explorer(occface, "vertex")
            occpts = py3dmodel.modify.occvertex_list_2_occpt_list(verts)
            pyptlist = py3dmodel.modify.occpt_list_2_pyptlist(occpts)
...
gshp_list = extract_shape_from_shapatt(grid_shpatt_list)
merged_grids = py3dmodel.construct.merge_faces(gshp_list)[0]

print "*******Reading Impervious File***************"
imp_list = read_sf_poly(imp_shp_file)
ishp_list = extract_shape_from_shapatt(imp_list)

id_imps = identify_srfs(merged_grids, ishp_list)
#id_imps=ishp_list
print "IDENTIFIED", len(id_imps)
...
for g in grid_shpatt_list:
    ...
    if uid in uid_list:
        ...
        id_imps2 = identify_srfs(grid, id_imps)
        #write_poly_shpfile(id_imps2, "F:\\kianwee_work\\princeton\\2019_06_to_2019_12\\campus_as_a_lab\\model3d\\shp\\main_campus_imps\\main_campus_imps_test.shp")
        
        print "IDENTIFIED2", len(id_imps2)
        #get the roads in the grid
        imps = common_road_frm_grid(grid, id_imps2)
        imps = redraw_faces(imps)
        imp_list2.extend(imps)
        
    write_poly_shpfile(imp_list2, shp_filepath)
    #py3dmodel.utility.visualise([imps])
    time2 = time.clock()
...
write_poly_shpfile(imp_list2, shp_filepath)
if viewer == True:
    py3dmodel.utility.visualise([imp_list2])