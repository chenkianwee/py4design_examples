...
shpfile = "F:\\kianwee_work\\case_study\\punggol_primary\\shp\\punggol_primary_clipped_svy21\\punggol_primary_clipped_svy21.shp"
#the shpfile_pt should only have one single point
shpfile_pt = "F:\\kianwee_work\\case_study\\punggol_primary\\shp\\punggol_primary_pt_svy21\\punggol_primary_pt_svy21.shp"
time1 = time.clock()
#specify the resultant collada file
collada_filepath = "F:\\kianwee_work\\case_study\\punggol_primary\\collada\\punggol_primary_extruded.dae"
...
occrec = pyliburo.py3dmodel.construct.make_rectangle(bdimx,bdimy)
...
if geo_ref == True:
    ...
    boundary_shell = pyliburo.py3dmodel.construct.make_shell([m_occrec])
    b_solid_list.append(boundary_shell)
    
if geo_ref == False:
    shp_in_boundary_cmpd = pyliburo.py3dmodel.modify.move(pypt3d, (0,0,0), shp_in_boundary_cmpd)
    b_solid_list = pyliburo.py3dmodel.fetch.geom_explorer(shp_in_boundary_cmpd, "solid")
    boundary_shell = pyliburo.py3dmodel.construct.make_shell([occrec])
    b_solid_list.append(boundary_shell)
    
time2 = time.clock()
time_spent = (time2-time1)/60.0
print time_spent
pyliburo.utility3d.write_2_collada(b_solid_list, collada_filepath)